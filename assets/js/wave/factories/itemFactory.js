app.factory('ItemFactory', function ($http, apiUrl, SweetAlert) {
    ...
    self.insertItemTag = function(item_tag, item){
        ...
        return $http.post(url + 'get_tag', item_tag)
            .success(function(data){
                if (data.call_status === "success") {
                    if (data.tag !== null) {
                        $data.tag_id = data.tag.tag_id;
                        if(self.isItemTagDuplicate(data.tag)){
                            SweetAlert.swal({
                                title: "Perhatian",
                            ...
                            $data.exist = "Y";
                            $http.post(url + 'insert_item_tag', $data).success(function(){
                                SweetAlert.swal({
                                    title: "Sussess",
