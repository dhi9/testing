app.controller('NewDeliveryController', function($scope, $state, $stateParams, $modal, ItemLookupService, VendorService, VehicleService,  ApiCallService, SweetAlert, CustomerFactory) {

	$scope.state = $state.current;
	
	//ORDER ID yang perlu dikirim
	...
	$scope.updateSelectedAddress = function(delivery_address_id, index){
        if(delivery_address_id != 0){
            for(var i = 0; i < $scope.deliveryAddressList.length; i+=1){
                if($scope.deliveryAddressList[i].delivery_address_id == delivery_address_id){
                    $scope.order.delivery_request_details[index].delivery_address = $scope.deliveryAddressList[i].delivery_address;
                }
            }
        }else{
            $scope.order.delivery_request_details[index].delivery_address = "";
        }
    }

    $scope.saveDelivery = function(delivery, index){

        delivery.order_id =  $scope.order.order_detail.order_id;
        var validasi = true;
        ...
        if (validasi) {
            ApiCallService.submitNewDelivery(delivery).success(function (data) {
                if (data.call_status === "success") {
                    $scope.order.delivery_request_details[index].delivery_id = data.delivery_id;
                    $scope.order.delivery_request_details[index].delivery_reference = data.delivery_reference;
                    $scope.order.delivery_request_details[index].HLPFIELD_is_edit_mode = false;
                    $scope.order.delivery_request_details[index].deliveries.push(delivery);
