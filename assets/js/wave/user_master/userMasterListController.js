app.controller('UserMasterListController', function($scope, $filter, UserService, ngTableParams, SweetAlert, $modal) {
		...
		UserService.getUserList().
			success(function(data, status, headers, config) {
				...
				if (data.call_status === "success") {
					$scope.usersList = data.user_list;
					
					for(var i = 0; i < $scope.usersList.length; i++){
						$scope.usersList[i].statusLabel = $scope.statusLabel($scope.usersList[i].status);
					}
