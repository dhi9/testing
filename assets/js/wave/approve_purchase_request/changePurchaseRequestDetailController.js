app.controller('changePurchaseRequestDetailController', function($filter, $scope, $http, $modal, $stateParams, $rootScope, PurchaseService, VendorService, ItemService, WarehouseService, SweetAlert, AttributeFactory, SiteService) {
	...
	$scope.searchSupplier = function(supplierName) {
		...
		if (found.length) {
			...
			$scope.purchase.supplier_email = $scope.supplier.sales_email;
...
*/			
	...
	$scope.loadItemRequest = function(index, item) {
		ItemService.getItemWithUomByItemCode(item.item_code).success(function(data){
			if (data.call_status === "success") {
				$scope.itemRequestList[index] = data.item_details;
				$scope.itemRequestList[index].quantity = item.quantity;
				$scope.itemRequestList[index].item_unit = item.item_unit;
				$scope.itemRequestList[index].cost = item.cost;
			}
		});
	}
