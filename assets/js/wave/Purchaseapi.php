...
class Purchaseapi extends CI_Controller {
	...
	public function __construct()
	{
		...
		$this->load->model(array('purchase_model', 'auditlog_model', 'user_model', 'site_model', 'purchase_bl', 'site_db', 'stock_movement_db', 'inventory_db', 'inventory_bl', 'vendor_model', 'warehouse_model', 'company_db'));
	...
	public function create_stock_card($requests_delivery_request_id){
	...
	public function insert_delivered_items()
	{
		...
		foreach ($data as $goodRecieved){
				foreach($goodRecieved['uom_list'] as $uom){
					if($uom['alternative_uom'] == $goodRecieved['item_unit']){
						$converted = (int)$uom['base_amount'] * $goodRecieved['quantity'];
						$count = 1;
						while((int)$converted > 0){
							...
							if(@$goodRecieved['auto_batch'] == true){
								$batch['batch_reference'] = $goodRecieved['mass_batch_reference'].$sequence;
							}else{
								$batch['batch_reference'] = @$goodRecieved['batch_reference'];
							}
							...
							if(array_key_exists("batch_limit", $goodRecieved)){
								$goodRecieved['batch_limit'] = $goodRecieved['batch_limit'];
							}else{
								$goodRecieved['batch_limit'] = $converted;
							}
							if($converted < $goodRecieved['batch_limit']){
								$quantity = $converted;
							}else{
								$quantity = $goodRecieved['batch_limit'];
							}
							if(@$goodRecieved['auto_batch'] == true){
								$insert_good_recieved['quantity'] = $quantity;
							}else{
								$insert_good_recieved['quantity'] = $quantity;
							}
							$insert_good_recieved['item_unit'] = $uom['base_uom'];
							...
							$purchase = $this->purchase_db->get_purchase_by_purchase_delivery_request_id($goodRecieved['requests_delivery_request_id'])->row_array();
							...
							$converted -= $goodRecieved['batch_limit'];
							$count += 1;
	...
	public function is_user_approver()
	{
		...
		if($num_rows){
			...
			$user = $this->purchase_model->get_users_by_user_id($user_id)->row()->$by;
			$users = $this->purchase_model->get_users_by_user_id($data['draft_approver'])->row()->$by;
			if($user > $users){
				$approver = true;
			}else{
				$approver = false;
