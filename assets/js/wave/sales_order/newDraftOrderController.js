app.controller('NewDraftOrderController', function($scope, $modal, ItemLookupService, ApiCallService, CustomerService, CustomerFactory, SweetAlert, $state, AuthService, AttributeFactory, CompanyFactory) {
	var changeColorOfOrderType = true;
	if(changeColorOfOrderType){
...
{"No":120,"bank_code":"'950'","bank_name":"PT. BANK COMMONWEALTH"}];
   ...
   $scope.combineAttributes = function(orderItems){
	...
	$scope.isDeliveryRequestValid = function(deliveryRequest) {
		...
		if (deliveryRequest.requested_delivery_date == "" || deliveryRequest.requested_delivery_date == null) {
			validity = false;
		}
		else {
	...
	$scope.insertDraftOrder = function() {
		...
		if ($test == true) {
			...
			else {$scope.submitButtonLoading = false;
				var inputOrder = {};
	...
	$scope.saveCart = function(){
	  if (!$scope.isCustomerValid()) {
		 SweetAlert.swal("Pilih Customer Terlebih Dahulu.");
	  }else{
		var inputOrder = {};
				angular.copy($scope.order, inputOrder);
				inputOrder.delivery_request_details.forEach(function(deliveryRequestDetail) {
		 ...
		 ApiCallService.insertDraftOrderCart(inputOrder).
					success(function(data, status, headers, config) {
						if (data.call_status === "success") {
	...
	$scope.getCustomerById = function(customer_id) {
