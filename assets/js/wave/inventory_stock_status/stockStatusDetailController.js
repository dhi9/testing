app.controller('stockStatusDetailController', function($http ,$scope, $filter, $modal, $state, $window, $stateParams, SiteService, ItemService, ngTableParams, SweetAlert, $timeout, $q, InventoryService) {
	$scope.tree_data = [];
	$scope.inventoryList = [];
	...
	InventoryService.getInventoryList(getInvent).success(function(data){
		if (data.call_status == 'success') {
			var tree;
